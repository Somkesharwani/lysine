...
public class CommonLibraryAutoConfiguration implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public CommonLibraryAutoConfiguration(Environment env, ObjectProvider<RequestMappingHandlerMapping> mappingProvider) {
        this.env = env;
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        ...
        if (mapping != null) {
            ...
            mapping.getHandlerMethods().forEach((key, value) ->
                    log.info("{} -> {}#{}", key.getMethodsCondition(), key.getPatternsCondition(), value.getMethod().getName())
            );
        } else {
